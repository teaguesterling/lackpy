...
When `@continue` is reached, execution stops and the interpreter returns partial results with `continue_requested: True` and all variables in scope. The agent loop can then feed these back to the model for the next iteration.
...
For multi-turn tasks, add `@continue` after the gather phase. The agent loop feeds variables back to the model, which writes the report in the next iteration.
