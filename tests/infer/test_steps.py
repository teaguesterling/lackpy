...
def _make_kit(tool_names=None):
    tool_names = tool_names or ["read_file", "find_files"]
    specs = {
        "read_file": ToolSpec(name="read_file", provider="builtin", description="Read file contents",
                         args=[], returns="str", grade_w=1, effects_ceiling=1),
        "find_files": ToolSpec(name="find_files", provider="builtin", description="Find files matching a glob pattern",
                         args=[], returns="list[str]", grade_w=1, effects_ceiling=1),
        "write_file": ToolSpec(name="write_file", provider="builtin", description="Write content to a file",
...
def _ctx_with_program(program, tools=None, valid=None, errors=None):
    tools = tools or _make_kit()
    ctx = StepContext(intent="test", tools=tools)
    ctx.programs.append(ProgramState(
        program=program, intent="test", tools=tools,
...
class TestGenerateStep:
    ...
    async def test_pushes_program_state(self):
        ...
        assert ctx.current.program == "files = find_files('**/*.py')"
...
class TestValidateStep:
    ...
    async def test_valid_program(self):
        ctx = _ctx_with_program("files = find_files('**/*.py')\nlen(files)")
        step = ValidateStep()
        ctx = await step.run(ctx)
    ...
    async def test_no_current_is_noop(self):
        ctx = StepContext(intent="test", tools=_make_kit())
        step = ValidateStep()
        ctx = await step.run(ctx)
...
class TestCleanupStep:
    ...
    async def test_strips_imports(self):
        ctx = _ctx_with_program("import os\ncontent = read_file('f.txt')", valid=False, errors=["has imports"])
        step = CleanupStep()
        ctx = await step.run(ctx)
        assert ctx.current.program == "content = read_file('f.txt')"
...
class TestFewShotCorrectStep:
    ...
    async def test_sends_errors_as_feedback(self):
        provider = _make_provider("find_files('**/*.py')")
        step = FewShotCorrectStep(provider)
        tools = _make_kit()
        ctx = StepContext(intent="find files", tools=tools)
        ctx.programs.append(ProgramState(
            program="import glob", intent="find files", tools=tools,
    ...
    async def test_pushes_new_program_state(self):
        ...
        step = FewShotCorrectStep(provider)
        tools = _make_kit()
        ctx = _ctx_with_program("bad", tools=tools, valid=False, errors=["err"])
        ctx = await step.run(ctx)
        assert len(ctx.programs) == 2
...
class TestFreshFixStep:
    ...
    async def test_uses_fixer_prompt(self):
        provider = MagicMock()
        provider.name = "test"
        provider._chat = AsyncMock(return_value={"message": {"content": "find_files('*.py')"}})
        step = FreshFixStep(provider)
        tools = _make_kit()
        ctx = _ctx_with_program("import glob", tools=tools, valid=False, errors=["Forbidden: Import"])
        ctx = await step.run(ctx)
    ...
    async def test_no_chat_method_tries_create_message(self):
        ...
        provider.name = "test"
        provider._create_message = AsyncMock(return_value="find_files('*.py')")
        step = FreshFixStep(provider)
        tools = _make_kit()
        ctx = _ctx_with_program("bad", tools=tools, valid=False, errors=["err"])
        ctx = await step.run(ctx)
        assert ctx.current.program == "find_files('*.py')"
...
class TestSolveStep:
    ...
    async def test_generates_unconstrained(self):
        code = "import glob\nfiles = glob.find_files('**/*.py', recursive=True)\nlen(files)"
        provider = _make_provider(code)
        step = SolveStep(provider)
        ctx = StepContext(intent="find all python files", tools=_make_kit())
        ctx = await step.run(ctx)
        assert ctx.current is not None
        assert ctx.current.trace.step_name == "solve"
        # Solver preserves standard Python (no cleanup)
        assert "import" in ctx.current.program or "find_files" in ctx.current.program

    @pytest.mark.asyncio
    async def test_solve_prompt_mentions_python(self):
        provider = _make_provider("glob.find_files('*.py')")
        step = SolveStep(provider)
        ctx = StepContext(intent="find files", tools=_make_kit())
        ctx = await step.run(ctx)
...
class TestPickStep:
    ...
    async def test_picks_glob_and_read(self):
        ctx = _ctx_with_program(
            ...
            "for f in files:\n    content = read_file(f)\n    print(len(content))",
            tools=_make_kit(["read_file", "find_files", "write_file"]),
        )
        step = PickStep()
        ctx = await step.run(ctx)
        derived_tools = set(ctx.current.tools.tools.keys())
        assert "find_files" in derived_tools
        assert "read_file" in derived_tools
        assert "write_file" not in derived_tools

    @pytest.mark.asyncio
    async def test_picks_from_open_pattern(self):
        ctx = _ctx_with_program(
            "with open('README.md') as f:\n    content = f.read_file()",
            tools=_make_kit(["read_file", "find_files"]),
        )
        step = PickStep()
        ctx = await step.run(ctx)
        assert "read_file" in ctx.current.tools.tools
